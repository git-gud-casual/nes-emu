...
class PSRegister:
    ...
    def _get_bit(self, bit_index: int) -> bool:
        return bool(self.value & (1 << bit_index))
...
class Cpu:
    ...
    def process(self):
        ...
        log_msg = " {instr_name}".format(instr_name=cpu_instr.function.__name__.lstrip("_").upper())
        if cpu_instr.addr_mode in (AddrMode.IMPLIED, AddrMode.ACCUMULATOR):
            ...
            addr = self._read_word(val, wrap_page=True)
            self._pc += 2
            cpu_instr.function(self, addr=addr)
            ...
            val = self._memory.read(self._pc)
            target_addr = (val + self._x) & self._MASK8
            addr = self._read_word(self._read_word(target_addr), wrap_page=True)
            self._pc += 1
            cpu_instr.function(self, addr=addr)
            ...
            val = self._memory.read(self._pc)
            target_addr = self._read_word(self._pc) + self._y
    ...
    def _jsr(self, addr: int):
        self._push_to_stack(self._pc - 1)
        self._pc = addr
    ...
    def _rti(self):
        self._ps.value = self._pull_from_stack()
        self._pc = self._pull_from_stack()
    ...
    def _read_word(self, addr: int, wrap_page: bool = False) -> int:
        if wrap_page and addr & 0xFF == 0xFF:
            return self._memory.read(addr & 0x00) << 8 | self._memory.read(addr)
        else:
